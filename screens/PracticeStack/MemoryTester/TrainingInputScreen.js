// screens/PracticeStack/MemoryTester/TrainingInputScreen.js — ввод ответов по позициям (числа или выбор из списка)
...
export default function TrainingInputScreen() {
  const route = useRoute();
  const navigation = useNavigation();
  const { sequence = [], moduleId, count, options: paramOptions } = route.params || {};
  ...
  const options = paramOptions ?? (moduleId === 'months' ? MONTHS : moduleId === 'days' ? DAYS : []);
  ...
  const onCheck = () => {
    ...
    navigation.navigate('TrainingResult', { correctCount: correct, total: n, errors });
  };
