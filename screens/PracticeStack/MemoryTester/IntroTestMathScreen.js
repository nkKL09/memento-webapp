// 5 примеров (+, −, ×, ÷), ответ 10–99, проверка по кнопке «Дальше»
...
export default function IntroTestMathScreen() {
  ...
  const onNext = () => {
    ...
    navigation.navigate('IntroTestInput', { sequence });
  };
        ...
        {tasks.map((task, i) => (
          <View key={i} style={[styles.row, wrongFlash.includes(i) && styles.rowFlash]}>
            <Text style={styles.task}>
              {task.a} {opSymbol(task.op)} {task.b} = ?
            </Text>
            <TextInput
